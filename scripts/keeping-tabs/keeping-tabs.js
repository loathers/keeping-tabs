...
var actions = {
  "mall this": item => (0,external_kolmafia_namespaceObject.putShop)(0, 0, item),
  "display this": item => (0,external_kolmafia_namespaceObject.putDisplay)((0,external_kolmafia_namespaceObject.itemAmount)(item), item),
  "use this": item => (0,external_kolmafia_namespaceObject.use)((0,external_kolmafia_namespaceObject.itemAmount)(item), item),
  "autosell this": item => (0,external_kolmafia_namespaceObject.autosell)((0,external_kolmafia_namespaceObject.itemAmount)(item), item)
};

function getItemsInTab(title) {
  (0,external_kolmafia_namespaceObject.print)("Matching tab ".concat(title));
  var tab = (0,external_kolmafia_namespaceObject.visitUrl)("inventory.php?which=f".concat(tabLookup[title]));
  var regexp = /ic(\d+)/g;
  var match;
  var items = [];
...
var fixedPrices = new Map([[$item(_templateObject || (_templateObject = _taggedTemplateLiteral(["pocket wish"]))), 49995]]);

function shopItems() {
  return Object.keys((0,external_kolmafia_namespaceObject.getShop)()).map(itemStr => (0,external_kolmafia_namespaceObject.toItem)(itemStr));
}

function main() {
  var args = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "use mall autosell display";
  var commandLookup = new Map([["mall", "mall this"], ["display", "display this"], ["use", "use this"], ["autosell", "autosell this"]]);
  var commands = args.split(" ").map(command => commandLookup.get(command));
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      if (command) {
        (0,external_kolmafia_namespaceObject.print)("Executing ".concat(command), "yellow");
  ...
  if (commands.includes("mall this")) {
    shopItems().forEach(item => {
      var fixedPrice = fixedPrices.get(item);

      if (fixedPrice) {
        (0,external_kolmafia_namespaceObject.repriceShop)(fixedPrice, item);
      }
    });
    (0,external_kolmafia_namespaceObject.cliExecute)("reprice");
    var maxPriceItems = shopItems().filter(i => (0,external_kolmafia_namespaceObject.shopPrice)(i) === 999999999);

    if (maxPriceItems.length > 0) {
      (0,external_kolmafia_namespaceObject.print)("Some Items are Still at Max Price", "red");
      maxPriceItems.forEach(i => (0,external_kolmafia_namespaceObject.print)("* ".concat(i), "red"));
    }
  }
}
